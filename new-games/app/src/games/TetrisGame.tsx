  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="grid grid-cols-3 gap-2 md:hidden">
        <div />
        <Button 
          onClick={() => { if (!gameOver && !isPaused) rotatePiece(); }} 
          variant="outline" 
          className="border-white/20 text-white h-14 text-lg"
          disabled={gameOver}
        >↻</Button>
        <div />
        <Button 
          onClick={() => { if (!gameOver && !isPaused) movePiece(-1, 0); }} 
          variant="outline" 
          className="border-white/20 text-white h-14 text-2xl"
          disabled={gameOver}
        >←</Button>
        <Button 
          onClick={() => { if (!gameOver && !isPaused) movePiece(0, 1); }} 
          variant="outline" 
          className="border-white/20 text-white h-14 text-2xl"
          disabled={gameOver}
        >↓</Button>
